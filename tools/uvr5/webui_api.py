...
for output in uvr(model_name, inp_root, save_root_vocal, paths, save_root_ins, agg, format0):
    print(output)

# 设置文件夹路径
directory_to_delete = save_root_vocal

# 过滤出以 'ins' 开头的文件
files_to_delete = glob.glob(os.path.join(directory_to_delete, "ins*"))
...
save_root_vocal="output/uvr5_opt2"
save_root_ins="output/uvr5_opt2"
for output in uvr(model_name, inp_root, save_root_vocal, paths, save_root_ins, agg, format0):
    print(output)
